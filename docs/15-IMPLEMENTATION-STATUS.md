...
- Initial merge rechecks each destination before adding a file: identical contents are accepted, and differing copies are preserved as conflicts without aborting unrelated transfers. The same check handles a file appearing at atomic commit. Case-only directory/file aliases across Windows/Linux manifests are reported before copying and at final verification; automatic case reconciliation remains unsupported.
