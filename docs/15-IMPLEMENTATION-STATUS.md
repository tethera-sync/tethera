...
- Removal of the scan-file ceiling itself. It is now a global preference (1,000–1,000,000 files, or explicitly unlimited; 10,000 by default) with no separate fixed per-side file-count cap gating legacy sync beneath it; every legacy scan still accumulates its full manifest in memory (now inspecting files concurrently and reusing cached digests) with cooperative cancellation, bounded admission (4 active, 16 queued), per-scan hash-buffer reuse, and the legacy full-manifest encoded-byte budget as the one remaining sync-side ceiling.
