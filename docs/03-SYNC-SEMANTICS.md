...
Bulk newest-copy resolution applies the same rules to a bounded queue batch. Each candidate is freshly hashed on both computers, and only a strictly later valid modification time selects a winner. Every copy Tethera installs carries its source file's modification time, set on the staged file before it is linked into place, so receiving a file never makes it look newer than the edit it came from. Copies installed before this was added kept their transfer time instead, and a version-control checkout or pull stamps old content with the current time, so bulk resolution remains a poor fit for working trees. Equal or unavailable times, changed or missing copies, and direction-blocked choices remain conflicts. Every engine batch records all exact choices in one SQLite transaction or records none; selected replacements still run through the ordinary archive-backed operation path.
...
Filesystem events are hints. Debounce bursts, probe stable size/mtime, hash with before/after metadata checks, retry changed/locked files and run periodic reconciliation scans. Because each check still walks the whole folder, a check started only by file changes rests for three times the previous check's duration (at most two minutes) after it; a user action or unfinished queued work skips the rest.
...
A changed cycle between computers that both advertise `scan-generations-v1` reconciles through staged generations instead of exchanging full manifests. Each computer stages its own full-integrity scan into SQLite as a sealed generation, fetches the other's sealed generation page by page, and reconciles the two set-based, so neither computer builds a full file list. A cycle whose durable operations include a conflict choice the user made stays on the manifest path, which owns replay and mirroring that choice to the paired computer; ordinary queued work is re-planned from the same observations and runs on the generation path. A peer without the capability always uses full manifests. Whichever path runs, the same baselines, directional durable operations, conflict rules and commit guarantees apply.

The initial merge uses the same staged generations between computers that both advertise `initial-merge-generations-v1`. Each pass stages both computers' scans and asks the engine for a read-only, paged additive plan: a one-sided remote file is a pull candidate unless its destination path is occupied or blocked, a same-path difference is a preserved conflict, and a local-only file belongs to the other direction. The plan writes no baseline, queues no operation and records no conflict, because an initial merge only becomes durable at activation. Exact totals on the first page gate free space before the first copy, and every page is consumed before the next is fetched, so neither computer holds a full plan. The three-step merge, its retry on files that changed during the copy, and the two-sided convergence check all use this plan; the convergence check fails when either direction still has a pull candidate. A peer without the capability, and every existing mapping, keeps the full-manifest merge unchanged.
...
Before a planned addition, recheck the destination. A regular file with the same size and full digest needs no transfer; differing content is preserved and reported as a conflict while other files continue. If an atomic no-replace commit finds a file that appeared during transfer, perform the same comparison after cleaning up staging. A destination that is already a folder, symbolic link, junction or other special entry, or that sits beneath a file or link, is never replaced or followed: scans report such occupied paths to both computers, and initial merge, final verification and continuous reconciliation leave the one-sided file out, report it as a warning and carry on with everything else. Before the first copy, the files planned for each computer must fit on its disk with 1 GiB kept spare; otherwise the merge stops with the sizes involved instead of filling a disk partway through. A source file removed, replaced or edited after the scan is left out while other files continue; final verification still requires any newer copy before activation. When final verification finds files that are still on only one computer, both merge passes run again from fresh scans (at most three attempts in total) so files created or changed during the copy are merged too; only then does the merge stop, naming the files that kept changing. Integrity, access and unsafe-path errors still fail closed. Fresh two-sided verification remains required before activation.
