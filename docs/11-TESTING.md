...
Existing-destination regressions cover identical-file reuse, preserving divergent versions regardless of timestamps, files appearing during commit, continued unrelated transfers, staging cleanup and integrity/symlink rejection. Rust and desktop comparison tests cover cross-participant directory-case collisions. Ignore-preset tests cover nested dependency exclusions, retained source/lockfiles, preserved custom rules and duplicate-free repeated application.
