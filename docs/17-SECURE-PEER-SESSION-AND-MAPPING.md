...
The coordinator may add `textDigest: true` to `continuous-sync-inspect-conflict`. The peer then also returns `textDigest`, the SHA-256 of its copy with every CRLF read as LF, taken from the same read as its exact digest, and omits it for binary content; older peers ignore the flag and older coordinators never send it. A copy is settled automatically only when both text digests match and exactly one copy's exact digest equals its text digest (it uses LF throughout). The peer's claim cannot change what is kept: a pulled copy must carry this computer's own text digest as its exact digest, which the transfer verifies, and a pushed copy is this computer's own file.
